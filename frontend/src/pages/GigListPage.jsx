...
const GigCard = ({ gig, isSaved, onToggleSave, user }) => {
  const statusConfig = {
  ...
  const cfg = statusConfig[gig.status] || { cls: 'text-slate-400 bg-white/5 border-white/10', dot: 'bg-slate-400' };

  const daysAgo = Math.floor((Date.now() - new Date(gig.created_at)) / (1000 * 60 * 60 * 24));

  const skillHints = ['React', 'Python', 'Node.js', 'TypeScript', 'AWS', 'Django', 'Vue', 'MongoDB', 'Flutter', 'Figma'];
  const gigSkills = skillHints.filter(s =>
    gig.title?.toLowerCase().includes(s.toLowerCase()) ||
      ...
      <div className="flex items-start justify-between gap-4 mb-4 relative z-10">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <span className={`flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full border ${cfg.cls}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${cfg.dot} ${gig.status === 'open' ? 'animate-pulse' : ''}`}></span>
            {gig.status === 'in_progress' ? 'Active' : gig.status === 'open' ? 'Open' : gig.status}
          </span>
          {user?.role === 'freelancer' && (
            <button
              onClick={(e) => { e.preventDefault(); onToggleSave(gig.gig_id); }}
      ...
      {gigSkills.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-6 relative z-10">
          {gigSkills.map((skill, i) => {
            ...
            return (
              <span key={skill} className="px-3 py-1 rounded-lg text-[12px] font-bold cursor-default"
        ...
        <div className="flex items-center gap-5 text-sm font-medium text-slate-400">
           <div className="flex flex-col gap-0.5">
              ...
              <div className="flex items-center gap-1">
                <span style={{ background: 'linear-gradient(135deg, #34d399, #10b981)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }} className="font-black">$</span>
                <span className="text-white font-bold">{Number(gig.budget).toLocaleString()}</span>
              </div>
            </div>
          <div className="flex flex-col gap-0.5">
  ...
  const fetchGigs = useCallback(async () => {
    ...
    try {
      const [gigsRes, savedRes] = await Promise.all([
        api.get('/gigs?status=open'),
        user?.role === 'freelancer' ? api.get('/gigs/saved').catch(() => ({ data: [] })) : Promise.resolve({ data: [] })
      ]);

      if (user?.role === 'freelancer') {
        setSavedGigs(new Set(savedRes.data.map(j => j.gig_id)));
      }

      let data = gigsRes.data;
...
`}</style>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mb-10 text-center sm:text-left">
          <p className="text-sm font-black mb-2 uppercase tracking-[0.18em]"
          ...
          <p className="text-slate-400 text-lg">
            Discover <span className="font-bold" style={{ color: '#818cf8' }}>{gigs.length}</span> exciting opportunities matching your skills.
          </p>
        </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-6 hidden lg:flex">
              <p className="text-sm font-medium text-slate-400">
                Found <span className="text-white font-bold">{gigs.length}</span> matching gigs
              </p>
            ...
            {loading ? (
              ...
              <div className="rounded-[32px] border border-white/10 border-dashed bg-white/[0.02] p-16 text-center">
                ...
                <p className="text-slate-400 text-sm font-medium mb-8 max-w-sm mx-auto leading-relaxed">
                  We couldn't find any gigs matching your current search criteria. Try adjusting your filters.
                </p>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {gigs.map(gig => (
                  <GigCard
                    ...
                    gig={gig}
                    user={user}
                    isSaved={savedGigs.has(gig.gig_id)}
