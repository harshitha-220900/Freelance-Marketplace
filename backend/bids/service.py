...
async def create_bid(db: AsyncSession, bid: BidCreate, freelancer_id: int) -> Bid:
    """Create a new bid for a specific job."""
...
async def accept_bid(db: AsyncSession, bid_id: int, client_id: int) -> Bid:
    """Accept a specific bid and reject all other bids for the same job."""
    ...
    if job.client_id != client_id:
        raise HTTPException(
            ...
            detail="You are not authorized to accept bids for this job"
        )
